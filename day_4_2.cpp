...
{ 
    ...
    for (int i = 0; i < lines.size(); i++)
    {
        ...
        for (int i = first_scope_begin_int; i <= first_scope_end_int; i++)
        {
            if (i >= second_scope_begin_int && i <= second_scope_end_int)
            {
                overlap_counter++;
                break;
            }
...
std::cout << "Same range: " << same_range_counter << std::endl;
std::cout << "Overlap: " << overlap_counter << std::endl;
